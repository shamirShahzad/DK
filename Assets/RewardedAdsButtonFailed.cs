...
namespace DK
{
    ...
    public class RewardedAdsButtonFailed : MonoBehaviour, IUnityAdsLoadListener, IUnityAdsShowListener
    {
        [SerializeField] Button _showAdButton;
        [SerializeField] string _androidAdUnitId = "Rewarded_Android";
        [SerializeField] string _iOSAdUnitId = "Rewarded_iOS";
        [SerializeField] PlayerManager player;
        [SerializeField] GameObject lowGoldPopup;
        ...
        public void OnUnityAdsShowComplete(string adUnitId, UnityAdsShowCompletionState showCompletionState)
        {
            if (adUnitId.Equals(_adUnitId) && showCompletionState.Equals(UnityAdsShowCompletionState.COMPLETED))
            {
                ...
                player.playerAnimatorManager.PlayTargetAnimation("Get Up",false);
                player.isDead = false;
        ...
        {
            if(FirebaseManager.instance.userData.goldAmount >= 500)
            {

                FirebaseManager.instance.UpdateGold(-500);
                deathPopup.SetActive(false);
                player.playerAnimatorManager.PlayTargetAnimation("Get Up", false);
                player.isDead = false;
                player.playerStatsManager.currentHealth = player.playerStatsManager.maxHealth;
            ...
            {
                StartCoroutine(displayLowGoldPopup());
            }
        }

        private IEnumerator displayLowGoldPopup()
        {
            lowGoldPopup.SetActive(true);
            yield return new WaitForSecondsRealtime(1.3f);
            lowGoldPopup.SetActive(false);
        }
